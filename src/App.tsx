    ...
    >
      ...
      >
        {mainMobileTabs.map(({ view, label, icon }) => (
          <button
            key={view}
            onClick={() => { handleNavClick(view); setShowMoreSheet(false); }}
        ...
        >
          <div
            className="w-8 h-7 rounded flex items-center justify-center text-base transition-all"
            style={{ background: moreIsActive || showMoreSheet ? 'var(--df-accent)' : 'transparent' }}
          >
            {showMoreSheet ? '✕' : '⋯'}
          </div>
      ...
      {showMoreSheet && (
        <>
          ...
          >
            <div style={{ width: 36, height: 4, borderRadius: 2, background: 'var(--df-border)', margin: '0 auto 12px' }} />
            <p style={{ fontSize: 11, color: 'var(--df-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>More</p>
              ...
              {moreMobileTabs.map(({ view, label, icon, badge }) => (
                <button
                  ...
                  style={{
                    ...
                    background: activeView === view ? 'rgba(79,110,247,0.15)' : 'var(--df-surface2)',
                  ...
                  {badge && (
                    <span
                      style={{
                        ...
                        background: 'var(--df-green)', color: '#fff',
