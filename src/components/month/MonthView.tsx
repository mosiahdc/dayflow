...
export default function MonthView({ monthDate, scheduledTasks }: Props) {
  ...
  // Build overflow map: nextDate → overflow continuation tasks
  const overflowMap = useMemo(() => {
    ...
    for (const st of scheduledTasks) {
      ...
      const nextDate = format(addDays(stDate, 1), 'yyyy-MM-dd');
      ...
      const continuation: ScheduledTask = {
        ...
        date: nextDate,
        startSlot: 0,
        task: { ...st.task, durationMins: remainingMins },
      };
      if (!map.has(nextDate)) map.set(nextDate, []);
      map.get(nextDate)!.push(continuation);
    }
    return map;
  }, [scheduledTasks]);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl border shadow overflow-hidden">
      ...
      <div className="grid grid-cols-7 border-b dark:border-gray-700">
        {DAY_HEADERS.map((d) => (
          <div
            key={d}
            className="text-center text-xs font-semibold text-brand-muted py-2 border-r dark:border-gray-700"
          >
            {d}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7">
        {cells.map((day) => {
          ...
          const overflow = overflowMap.get(dateStr) ?? [];
          return (
            <MonthCell
              key={dateStr}
              date={dateStr}
