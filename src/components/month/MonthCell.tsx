...
interface Props {
  ...
  scheduledTasks: ScheduledTask[]; // tasks starting this day
  overflowTasks: ScheduledTask[]; // continuations from previous day
}

export default function MonthCell({ date, isCurrentMonth, scheduledTasks, overflowTasks }: Props) {
  ...
  const allTasks = [...overflowTasks, ...scheduledTasks];
  const visible = allTasks.slice(0, 3);
  const extra = allTasks.length - visible.length;

  return (
    <div
      className={`min-h-[100px] border-r border-b dark:border-gray-700 p-1 flex flex-col
          ...
          return (
            <div
              ...
              style={{
                ...
                opacity: isOverflow ? 0.75 : 1,
              }}
            >
              {isOverflow ? `↩ ${st.task.title}` : st.task.title}
            </div>
          );
        })}
        {extra > 0 && <p className="text-xs text-brand-muted">+{extra} more</p>}
      </div>

      {showPicker && <MiniTaskPicker date={date} onClose={() => setShowPicker(false)} />}
    </div>
  );
}
